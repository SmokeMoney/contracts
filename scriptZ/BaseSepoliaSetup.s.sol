...
contract SetupScript is Script {
    ...
    function run() external {
        ...
        AdminDepositContract depositBAS = AdminDepositContract(deposit_BAS_Address);
        CrossChainLendingContract lendingBAS = CrossChainLendingContract(lending_BAS_Address);

        depositBAS.addSupportedToken(wsteth_BAS_Address);
        depositBAS.addSupportedToken(weth_BAS_Address);

        lendingBAS.poolDeposit{value: 0.5*1e18}(0.5*1e18);

        depositBAS.setPeer(ARBEID, addressToBytes32(opsCotnractAddress));
        vm.stopBroadcast();
    }
