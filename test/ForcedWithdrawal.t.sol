...
contract DepositTest is Setup {
    ...
    function testForcedWithdrawal() public {
        ...
        vm.startPrank(user2);
        lendingcontractB.borrow(issuer1NftAddress, tokenId, uint256(0.5 * 10**18), timestamp, signatureValidity, uint256(0), false, signature);
        vm.stopPrank();

        vm.warp(1720963281);
        timestamp = vm.getBlockTimestamp();
        digest = keccak256(abi.encodePacked(user, tokenId, uint256(0.1 * 10**18), timestamp, signatureValidity, uint256(0), uint256(cEid)));
        signature = getIssuersSig(digest); 

        vm.startPrank(user);
        lendingcontractC.borrow(issuer1NftAddress, tokenId, uint256(0.1 * 10**18), timestamp, signatureValidity, uint256(0), false, signature);
        vm.stopPrank();
        ...
        vm.startPrank(user);
        accountOps.forcedWithdrawal{value: sendFee.nativeFee}(assembleId, addressToBytes32(address(weth)), withdrawAmounts, targetChainIds, extraOptions, addressToBytes32(address(user)));
        vm.stopPrank();
        verifyPackets(bEid, addressToBytes32(address(depositCrossB)));

        assertEq(depositCrossB.getDepositAmount(address(weth), issuer1NftAddress, tokenId), 0.31 * 1e18);
        ...
        vm.startPrank(user);
        vm.expectRevert();
        accountOps.forcedWithdrawal{value: sendFee.nativeFee}(assembleId, addressToBytes32(address(wstETH)), withdrawAmounts, targetChainIds, extraOptions, addressToBytes32(address(user)));
        vm.stopPrank();
