...
contract DepositTest is Setup {
    ...
    function testWithdrawCrossChain() public {
        ...
        aEnforcedOptions[0] = EnforcedOptionParam({
            eid: bEid,
            msgType: SEND,
            options: OptionsBuilder.newOptions().addExecutorLzReceiveOption(90000, 0)
        }); // gas limit, msg.value
        ...
        bytes memory payload =
            abi.encode(address(user), address(weth), tokenId, 0.1 * 10 ** 18, accountOps.withdrawalNonces(tokenId));

        MessagingFee memory sendFee =
            accountOps.quote(bEid, SEND, accountOps.encodeMessage(1, payload), extraOptions, false);
