...
contract SetupScript is Script {
    ...
    function run() external {
        vm.startBroadcast();
        AdminDepositContract depositOPT = AdminDepositContract(deposit_OPT_Address);
        CrossChainLendingContract lendingOPT = CrossChainLendingContract(lending_OPT_Address);

        depositOPT.addSupportedToken(weth_OPT_Address);
        depositOPT.addSupportedToken(wsteth_OPT_Address);

        lendingOPT.poolDeposit{value: 0.5*1e18}(0.5*1e18);

        depositOPT.setPeer(ARBEID, addressToBytes32(opsCotnractAddress));
        vm.stopBroadcast();
    }
