...
contract OperationsContract is EIP712, Ownable, OApp, OAppOptionsType3 {
    ...
    function _validateWithdrawSignature(WithdrawParams memory params, bytes memory signature) internal view {
        ...
        require(
            SignatureChecker.isValidSignatureNow(issuers[params.issuerNFT], digest, signature),
    ...
    function forcedWithdrawal(
        ...
        (uint256 totalBorrowed, uint256 totalCollateral) =
            assemblePositionsContract.calculateTotatPositionsWithdrawal(assembleId, assemble.nftId);
        // Calculate total available to withdraw
        uint256 maxBorrow =
            (totalCollateral * nftContract.getLTV() / 10000) + nftContract.getNativeCredit(assemble.nftId); // TODO @attackvector can this be misused? the extra limit I mean
        console2.log(
            "totalBorrowed: %s | maxBorrow: %s | totalCollateral: %s", totalBorrowed, maxBorrow, totalCollateral
    ...
    function _executeWithdrawals(
        ...
        for (uint256 i = 0; i < amounts.length; i++) {
            uint256 oracleMultiplier = token
                == assemblePositionsContract.getAssembleWstETHAddresses(assembleId, targetChainIds[i])
                ? assemble.wstETHRatio
                : 1e18;

            require(assemble.totalAvailableToWithdraw >= amounts[i], "Insufficient funds to withdraw");
            uint256 updatedTotalAvailableToWithdraw =
    ...
    function liquidationChallenge(
        ...
        (uint256 lowestAssembleTimestamp, uint256 latestBorrowTimestamp) =
            assemblePositionsContract.calculateTimestamps(assembleId, gAssembleIds);
