...
contract DepositTest is TestHelperOz5 {
    ...
    function testBorrow() public {
        ...
        console.log("Lending  pos", lendingcontract.getLendPosition(tokenId,  address(user2)));
        console.log("");
        console.log("Before we started");
        ...
        vm.startPrank(user3);
        lendingcontract.repayOrLend{value: 25 ether}(tokenId, address(user2));
        vm.stopPrank();
        console.log("");
        console.log("user debt", lendingcontract.getNetPosition(tokenId, user2)); 
        console.log("Borrow pos", lendingcontract.getBorrowPosition(tokenId, user2));
        ...
        console.log("Lending  pos", lendingcontract.getLendPosition(tokenId,  address(user2)));
        vm.startPrank(user3);
        lendingcontract.repayOrLend{value: 25.500017202568039109 ether}(tokenId, address(user2));
        vm.stopPrank(); 
    ...
    function testBorrow2() public {
        ...
        uint256 timestamp = vm.getBlockTimestamp();
        vm.startPrank(issuer);
        bytes32 digest = keccak256(abi.encodePacked(user2, tokenId, uint256(5 * 10**18), timestamp, uint256(1), uint256(adminChainIdReal)));
        bytes32 hash = siggen.getEthSignedMessageHash(digest);
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(issuerPk, hash);
        bytes memory signature = abi.encodePacked(r, s, v); // note the order here is different from line above.
        vm.stopPrank();
        ...
        // Mint an NFT for the user
        vm.startPrank(user2);
        lendingcontract.borrow(tokenId, uint256(5 * 10**18), timestamp, uint256(1), signature);
        vm.stopPrank();
    }
    ...
    function testRepay() public {
        vm.startPrank(user3);
        lendingcontract.repayOrLend{value: 0.1 ether}(tokenId, address(user2));
        vm.stopPrank();
    ...
    function testMultiRepayment() public {
        ...
        lendingcontract.repayOrLendMultiple{value: totalBorrowed}(nftIds, walletAddresses, borrowAmounts);
